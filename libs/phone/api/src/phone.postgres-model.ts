import { fn, NonAttribute } from 'sequelize';
import {
  Table,
  Column,
  Model,
...
export class PhoneModel extends Model<PhoneModel> {
  ...
  @Column(
    new DataType.VIRTUAL(DataType.STRING, [
      'countryCode',
      'phone',
      'regionCode',
    ])
  ...
  @Column(
    new DataType.VIRTUAL(DataType.BOOLEAN, ['verifiedAt', 'twilioCodeSentAt'])
  )
  get isSent(): boolean {
    return (
      false ||
      !!this.getDataValue('verifiedAt') ||
      this.getDataValue('twilioCodeSentAt') >
