...
export const TableComponent: React.FC<TableComponentProps> = React.forwardRef(
  (props, ref) => {
    const {
      changeLimit,
      changeOffset,
    ...
    return (
      ...
      >
        <Collapse in={isInitialized}>
          <Box padding="0" ref={ref} width="100%">
            <TableContainer
              component={Box}
              style={{
                maxHeight,
                    ...
                    {header.map((data: TableHeaderItem, index: number) => {
                      return (
                        <StyledTableCell
                          ...
                          sortDirection={
                            orderBy === data.fieldName ? order : false
                          }
                        ...
                        >
                          {data.sortable ? (
                            <StyledTableSortLabel
                              ...
                              direction={
                                orderBy === data.fieldName ? order : 'asc'
                              }
                              onClick={() =>
                                typeof changeSort === 'function' &&
                                changeSort(data.fieldName)
                              }
                            >
                              {data.title}
                            </StyledTableSortLabel>
                ...
                <TableBody>
                  ...
                  {!loading &&
                    ...
                    rows.map((row: TableRowType) => {
                      const clickable =
                        !!clickRow && typeof clickRow === 'function';
                      return (
                        <StyledTableRow
                          key={row.id}
                          loading={loading ? 'true' : ''}
                        ...
                        >
                          {row.columns.map(
                            (cell: TableCellData, index: number) => {
                              return (
                                <TableCell
                                ...
                                >
                                  <Fade in={true} timeout={FADE_TIMEOUT_DUR}>
                                    <span>
                                      {cell.componentType === 'text' &&
                                        (cell.data as string)}
                                      {cell.componentType === 'icon' &&
                                        !!cell.icon &&
                                        renderIcon(cell.icon, cell.color)}
                                    </span>
                                  </Fade>
                                </TableCell>
                              );
            ...
            <Table>
              ...
              >
                <TableRow>
                  <TablePagination
                    rowsPerPageOptions={
                      count > 10 ? rowsPerPageOptions : undefined
                    }
                    colSpan={header.length + 1}
                    count={count}
