...
export class UserModel extends Model<UserModel> {
  ...
  async getEmails(): Promise<EmailModelType[]> {
    this.emails =
      null || this.emails || (await EmailModel.findAllByUserId(this.id));
    return this.emails;
  }
  ...
  async getPhones(): Promise<PhoneModelType[]> {
    this.phones =
      null || this.phones || (await PhoneModel.findAllByUserId(this.id));
    return this.phones;
  }
   ...
   *
   */
  async fetchConnectedDeviceBeforeToken(
    token: string
  ...
  static async registerAndCreateFromEmail(
    ...
    await EmailModel.createOrFindOneByUserId(
      user.id,
      email,
      token,
      shouldValidate
  ...
  static async registerAndCreateFromPhone(
    ...
    await PhoneModel.createOrFindOneByUserId(
      user.id,
      phone,
      countryCode,
      regionCode,
  ...
  static async createFromUsername(
    ...
    if (
      phone &&
      countryCode &&
      !(await PhoneModel.isPhoneAvailable(phone, countryCode))
    ) {
    ...
    try {
      ...
      const [emailData, didCreateEmail] =
        await EmailModel.createOrFindOneByUserId(
          user.id,
          email,
          token,
      ...
      if (phone && countryCode) {
        const [phoneData, didCreatePhone] =
          await PhoneModel.createOrFindOneByUserId(
            user.id,
            phone,
            countryCode,
