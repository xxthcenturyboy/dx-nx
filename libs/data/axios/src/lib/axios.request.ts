...
export class RequestClass {
  constructor() {
    this.baseUrl = settings.APP_HOST ? `${settings.APP_HOST}:${settings.APP_PORT}/api` : '/api';
  ...
  private _handleNotification (): void {
    ...
    if (state.router.location.pathname !== allRoutes.main) {
      ...
      if (!state.app.isShowingUnauthorizedAlert) {
        ...
        toast.warn(msg, {
          onClose: () => store.dispatch(appActions.setIsShowingLogoutAlert(false))
        });
      }
    }
  }
  ...
  public async fireRequest(config: AxiosRequestConfig, fromLogoutDispatch?: boolean): Promise<RequestResponse> {
    if (!config) {
    ...
    try {
      ...
      if (
        ...
        if (
          (response as AxiosResponse).status === 400
          && this.includeAlerts
        ) {
          store.dispatch(appActions.setAppDialog(null));
          store.dispatch(appActions.setApiDialog(this.debug ? message : ERROR_MSG_API));
        }
